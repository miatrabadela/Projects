[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/BBG1WJ2I)
...
`void disp()`\
➜ Print the contents of the stack. Begin by printing the index of the top of your stack like so:
```
cout << "top = " << top << endl;
```
Next, print each value in the stack array on a separate line.
...
1. `pancakesCooked`: tracks the total number of pancakes cooked by the restaurant
2. `profit`: the total profit made by the pancake house
3. `pancake_types[]`: an array containing the costs of the different types of pancakes the store offers. This array is initialized at the top of `PancakeHouse.cpp`. Note that the pancake types are the numbers 1 - 5, where the cost of pancake type 1 corresponds to the first entry in the `pancake_types` array, and so on.
...
➜ Sets `profit` and `pancakesCooked` to `0`. Allocates new instances of `cashRegister` and `shopQueue` on the heap. 

`~pancakeHouse()` - Destructor\
➜ Deallocate the `cashRegister` and `shopQueue` objects. Handle dynamically allocated memory properly.

`void addOrder()`\
➜ Use the corresponding ShopQueue function to add a customer order to the queue. Make sure that the customer's pancake order is for a valid pancake type (numbers 1 through 5).

`void cookPancakes()`\
➜ Process the next order in the queue, if one exists. Increment the number of pancakes cooked according to the amount of pancakes in the customer's order. Add a ticket to the register with the total cost of the order given by `number_of_pancakes * cost_of_pancake_type`. Once the order is processed, remove the customer from the queue. **Do not** update the profit variable. If there are no customers in the queue, your function shouldn't do anything.  

`void strikeOrder()`\
➜ Remove the next order in the queue. 

`void refundOrder()`\
➜ Remove the top ticket in the register and print:
```
Money refunded = [refund]
```
followed by a line break, where [refund] is the amount refunded. **Do not** update the profit variable. If there is no ticket in the register, print:
```
...
`void closeShop()`\
➜ Traverse through the shop queue and the register, removing all the items in each. Do not deallocate the shop queue or cash register objects. 

`void updateTotalProfit()`\
➜ Remove the tickets that are in the cash register, adding the money associated with each ticket to the `profit` member variable. 

`void printOrders()`\
➜ Traverse through the queue of customer orders and print out all the information in each order. 
...
```
followed by a line break after each piece of information. Make sure that the customers remain in the shop queue in their original order after this method runs. If there are no customers in the queue, do not print anything.
